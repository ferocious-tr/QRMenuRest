...
def show_order_history():
    ...
    for order in active_orders:
        ...
        with st.expander(f"{status_info['icon']} Sipariş #{order.order_number} - {status_info['label']}", expanded=True):
            ...
            # Rating system for served orders
            if order.status == 'served' and not order.rating:
                st.markdown("---")
                st.markdown("### ⭐ Siparişinizi Değerlendirin")
                
                cols = st.columns(5)
                for i, col in enumerate(cols, 1):
                    with col:
                        if st.button(f"{'⭐' * i}", key=f"rate_{order.id}_{i}", use_container_width=True):
                            # Update rating in database
                            try:
                                order.rating = i
                                db.session.commit()
                                st.success(f"Teşekkürler! {i} yıldız verdiniz.")
                                time.sleep(1)
                                st.rerun()
                            except Exception as e:
                                st.error(f"Değerlendirme kaydedilemedi: {e}")
            
            elif order.status == 'served' and order.rating:
                st.success(f"✅ Değerlendirmeniz: {'⭐' * order.rating}")
...
if __name__ == "__main__":
    ...
    # Safe auto-refresh timer - runs after all content is displayed
    # Only refreshes to update order statuses, doesn't interfere with cart display
    if st.session_state.get('table_id'):
        # Create a placeholder at the bottom of the page
        timer_placeholder = st.empty()
        
        # Check if there are any active orders to track
        db = get_db()
        from database.models import Order
        from sqlalchemy import and_
        ...
        # Only run timer if there are active orders to track
        if active_orders > 0:
            ...
            # Clear the timer message before refresh
